...
from utils import (write_multi, read_multi, ENEMY_TABLE,
                   ...
                   make_table, utilrandom as random, ENEMY_SKILLS_TABLE,
                   VANILLA_ENEMY_SCRIPTS_TABLE)
...
xps = []
gps = []
# http://www.rpglegion.com/ff6/hack/command.htm
...
class MonsterBlock:
    def __init__(self, name, monster_id):
        ...
        #these are not set until get_formations, lookup_enemies
        #are called via formationrandomizer in randomizer.py
        ...
        #### added for WC testing
        self.skill_candidates = []
        self.vanilla_aiscript = None
    ...
    def choose_graphics(self, candidates):
        ...
        #min/maxy = minimum/maximum y pos of monster in any mould?
        
        #check if large bit, x-y position, width/height
        ...
        if not self.graphics.large:
            #don't pick miny smaller than current+1
            if self.miny <= 3:
                candidates = [c for c in candidates if c.miny <= 4]
            else:
                #allow miny current-1 and up
                candidates = [c for c in candidates if c.miny >= 3]
            #don't pick maxy smaller than current
            if self.maxy >= 13:
                candidates = [c for c in candidates if c.maxy >= 13]
            else:
                #don't pick maxy larger than current
                candidates = [c for c in candidates if c.maxy <= 12]

        #review candidates for enemy sprites that are too large along the bottom edge
        # of the y axis
    ...
    def mutate_graphics_swap(self, candidates):
        ...
        #print("SWAP %s (%s) <-> %s (%s)" % (self.name, self.graphicname,chosen.name, chosen.graphicname))
        a, b = self.graphicname, chosen.graphicname
        self.graphicname, chosen.graphicname = b, a

        self.graphics.swap_data(chosen.graphics)

        #WC - sets flags based on humanoid, undead, floating?
    ...
    def read_stats(self, filename):
        ...
        try:
            ...
            #rages are 2 spell IDs. see spellcodes.txt
            if not self.is_boss:
                f.seek(self.rageptr)
            ...
            self.read_ai_vanilla(filename)
        except Exception as e:
            traceback.print_exc()



    #skillset pulls out any ability within the AI script, Battle, Special,
    #when this pulls out "WC tiered spell" abilities they are still
    #pointers to the vanilla spell indices, so even though they display
    #wrong in the control menu they will still cast what you see in usME
    #could also reference a vanilla scripts table so it doesn't
            #bring in WC replaced scripts
            #WC - monster skillsets
    #use a new flag "vanilla"
    def get_skillset(self, ids_only=True, vanillaSkills=False):
        def monster_skills_from_table(tablefile, monsterid):
            for i, line in enumerate(open(tablefile)):
                monster_skills = []
                line = line.strip()
                line = line.split(':')
                if int(line[0]) == monsterid:
                    skills = line[1][1:].split(', ')
                    for x, value in enumerate(skills):
                        monster_skills.append(int(value))
                    break
                #print(monster_skills)
                #input("x")
            return monster_skills

        if vanillaSkills:
            skillset = monster_skills_from_table(ENEMY_SKILLS_TABLE, self.id)
        else:
            skillset = set([])
            skillset.add(0xEE)
            skillset.add(0xEF)
            skillset.add(0xFE)   #this is listed as Lagomorph in spellcodes.txt? Why would this be added?
            for action in self.aiscript:
    ...
    def mutate_ai(self, Options_, change_skillset=True, safe_solo_terra=True,vanillaControlFlag=False):
        ...
        skillset = set(self.get_skillset(vanillaSkills=vanillaControlFlag))
        ...
        ### WC mod to restrict more: BabaBreath, Zinger, Desperation Attacks
        restricted = [0x13, 0x14, 0xEA, 0xDA, 0xF0, 0xF1, 0xF2, 0xF3, 0xF4, 0xF5, 0xF6, 0xF7, 0xF8, 0xF9, 0xFA, 0xFB, 0xFC, 0xFD]
        #print(restricted)
        ### input("X")

        banned = restricted

        #WC - not relevant here
        ...
        targeting = None
        newscript = []
        #WC - each action is a bytearray, format:
        #[bytearray(b'\xf0\xee\xee\xef'), bytearray(b'\xff'), bytearray(b'\xff')]
        #action[0] & 0xF0 == 0xF0 means the first value in this array is 0xF<something>
        for action in self.aiscript:
            if action[0] & 0xF0 == 0xF0:
                #F1: set target
                #FC: conditional attacks
                if action[0] in [0xFC, 0xF1]:
                    targeting = random.choice([True, None])
                    # 2/3 chance, if conditional attack
                    if action[0] == 0xFC and random.randint(1, 3) != 3:
                        if action[1] == 0x04:
                            # if hit by element
                            nulled = self.absorb | self.null
                            # nulled is either the enemy absorbs or nullifies the element
                            if action[2] & nulled:
                                # conditional is swapped to -> if enemy is hit with an element that targets weakness
                                if self.weakness:
                        ...
                        if action[1] == 0x06:
                            ...
                            # modify the HP below value point. not gonna work with scaled HP in WC
                            action[3] = value
                        if action[1] in [0x0B, 0x16]:
                            ...
                            value = min(0xFF, max(0, value))
                            action[2] = value
                            # modify the timer value, probably skip this for WC,
                            # should depend more on the context
                        if action[1] in [0x0C, 0x0D]:
                            ...
                            action[3] = value
                            # find examples of this. heavily dependent on what's in the second byte spot
                            # prob skip this for WC

                #F6: throw or use
                # in general modifying weapons thrown/used can get pretty wonky
                elif action[0] == 0xF6 and not itembreaker:
                    items = get_ranked_items()
                    if action[1] == 0x00:
                        # use item. the 'not i.features['targeting'] & 0x40' part excludes Magicite and Super Ball?
                        # shouldn't this bypass Rename Cards as well?
                        ...
                        # throw item. sort throwable weapon items based their bat power
                        # not sure how this handles non-throwable weapons?
                    ...
                    for i, a in enumerate([first, second]):
                        ...
                        ### WC mod to prevent Warp Stone/Smoke Bombs/Megalixirs (convert to Remedy)                   
                        if candidates[index].itemid == 0xFD or candidates[index].itemid == 0xFC or candidates[index].itemid == 0xEF:
                            candidates[index].itemid = 0xF5
                        ### likely problems in some situations if Warp/Smoke Bomb is allowed. Other items might be
                        ### problematic as well
                            
                        action[i + 2] = candidates[index].itemid

                # FD - Wait until attack sequence is called upon again, then continue (reset targeting)
                ...
                #F0: use an ability script (1/3 chance of using an ability)
                elif action[0] == 0xF0:
                    #spell picker
                    #itembreaker not on for WC mod
    ...
    def read_ai_vanilla(self, filename):
        try:
            f = open(filename, 'r+b')
            ...
            while True:
                ...
                try:
                    ...
                    #WC - add section to read in values
                    #if desc_val == 240:
                    #if numargs == 3:
                    #for x in range(numargs):
                        #arglist.append(hex(ord(f.read(1))))
                        #arglist.append(f.read(1))
                except KeyError:
                    args = b""

                #print(str(hex(desc_val)) + ": " + str(arglist))
                #print(arglist)

                #WC - comment out
                script.append(value + args)
                #script.append(str(hex(desc_val)) + str(arglist))
            ...
            #input("script done")
            def vanilla_script_from_table(tablefile, monsterid):
                for i, line in enumerate(open(tablefile)):
                    monster_script = []
                    line = line.strip()
                    line = line.split(': ')
                    if int(line[0]) == monsterid:
                        #scripts = line[1][1:].split(', ')
                        scripts = line[1].split(', ')
                        #print(scripts)
                        #input("X")
                        for x, value in enumerate(scripts):
                            #print(value)
                            str_len = len(value)
                            #print(str_len)
                            tmpbytes = b''
                            ...
                            #for i in range(str_len):
                                ...
                                if i <= str_len:
                                    #print(testme[i:i+2])
                                    if value[i:i+2] == "\\x":
                                        #convert 2 characters after \x substring into bytes
                                        #input("found \\x")
                                        #print(testme[i+2:i+4])
                                        tmpbytes = tmpbytes + bytes.fromhex(value[i+2:i+4])
                                        #input("found \\x done")
                                        #print(i)
                                        i = i + 3
                                        #print(i)
                                    elif value[i] == "\\":
                                        #convert next 2 characters into bytes
                                        #print(str(value[i:i+2]))
                                        if value[i:i+2] == '\\r':
                                            tmpbytes = tmpbytes + bytes(b'\r')
                                        elif  value[i:i+2] == '\\t':
                                            tmpbytes = tmpbytes + bytes(b'\t')
                                        elif  value[i:i+2] == '\\n':
                                            tmpbytes = tmpbytes + bytes(b'\n')
                                        elif  value[i:i+2] == "\\'":
                                            tmpbytes = tmpbytes + bytes(b'\'')
                                        ...
                                        #input("x")
                                        #print(str(value[i:i+2]))
                                        #if str(value[i:i+2]) == '\r':
                                            #input("A")
                                        
                                        #print(ord(value[i:i+2]))
                                        #print(hex(ord(value[i:i+2])))
                                        #input("hold")
                                        #tmpbytes = tmpbytes + bytes(hex(ord(value[i:i+2])))
                                        #tmpbytes = tmpbytes + bytes(value[i:i+2].encode())
                                        i = i + 1
                                    else:
                                        #if value[i] == ":":
                                            #input("airforce")
                                        ...
                                        #print(i)
                                        #input("single")
                                        #convert next character into bytes
                                        tmpbytes = tmpbytes + bytes(value[i].encode())
                                else:
                                    pass
                                    #with only 2 characters left
                            ...
                            #print(monsterid)
                            #print(tmpbytes)
                            #input("next value in script")

                            monster_script.append(bytearray(tmpbytes))
                        #end of for loop
                        break
                return monster_script

            #if vanilla:
            script = vanilla_script_from_table(VANILLA_ENEMY_SCRIPTS_TABLE, self.id)
                        
            if self.id == 0xF3:
                # account for brown Mag Roader bug
                script = script[:4]

            self.vanilla_aiscript = script

            return self.vanilla_aiscript
        except Exception as e:
            traceback.print_exc()   
    ...
    def write_stats(self, fout):
        ...
        #######for WC we don't want to mess up Rages which means Specials need to be "normal"
        ...
        ####### If mutate_control was not called,
        # self.rages won't be any different, so this will be redundant
        if not self.is_boss:
    ...
    #monster ID's: 79, 189, 173
    #Bomb, SoulDancer, Outsider
...
#makes some bosses escapable? makes some enemies otherwise not escapable "escapable"?
#randomizes whether enemy is scannable?
#randomizes whether enemy is undead?
...
#modifies level, all stats, hp/mp/xp/gp
#check if WC level scaling STILL APPLIES AFTER THIS INITIAL LEVEL CHANGE. looks like it does
    def mutate_stats(self, monsterstats1="n",monsterstats2="n"):
        level = self.stats['level']
        ...
        #WC - vary stats
        if monsterstats1 == "y":
            #stat_order = ['speed', 'attack', 'hit%', 'evade%', 'mblock%','def', 'mdef', 'mpow']
            for stat in stat_order:
                if stat == 'speed':
                    limit = random.randint(170, 230)
                    self.stats[stat] = limit
                    #limit = 230 #Because Speed + 20 must fit in a single byte, any speed over 235 would result in a very slow-moving ATB gauge.
                else:
                    limit = 0xFF
                ### WC mod week 3 - only boost speed
                if 0 == 1:
        ...
        #WC - vary HP/MP
        if monsterstats2 == "y":
            #made this int to handle easybossupgrades
            self.stats['hp'] = level_boost(int(self.stats['hp']), limit=0x10000)

            #WC - comment out for WC
            #if HP is maxxed out apply Life3 status
            if self.stats['hp'] == 0x10000:
                #self.statuses[3] |= 0x04  # reraise
                #force max HP to 65535
                self.stats['hp'] = 0xFFFF

            self.stats['mp'] = level_boost(self.stats['mp'], limit=0xFFFE)
        #####end of WC stat variation


        #handles XP/GP, probably can take this out
        ...
        #more level variance, probably unnecessary
    ...
    def mutate_affinities(self, odds=10):
        ...
        #WC - comment out low level check for nullification
        #if self.stats['level'] < 20 and nullify & 0x7 == 0x7:
            #denullify = 0xFF ^ (1 << random.randint(0, 2))
            #self.null &= denullify
            #self.absorb &= denullify

    def mutate_items(self, katnFlag=False, monsterdrops="n", monstersteals="n",
                     exclusion_list=[]):
        #if random.choice([True, False]):
        ...
        if len(exclusion_list) > 0:
            items_after_exclusion = [ele for ele in items if ele.itemid not in exclusion_list]
            items = items_after_exclusion

        itemids = [i.itemid for i in items]

        #####WC - initialize steals and drops to vanilla values
        #OGsteals_names = [i.name for i in self.steals if i]
        #OGdrops_names  = [i.name for i in self.drops if i]
        #print("Drops: " + str(OGdrops_names))  #prints rare, then common
        #print("Steals: " + str(OGsteals_names))  #prints rare, then common
        
        OGsteals = self.steals
        OGdrops = self.drops
        #####WC - initialize steals and drops to vanilla values
        ...
        for i in self.items:
            ...
            #using the ranked item list adjusts the index within that list (this is a form of tiering)
            #print("old: " + str(index))
        ...
        #puts rarer items in rare slots
        ...
        #for i in range(len(steals)):
            #if not(steals[i] is None):
                #print(steals[i].name)
        
        #print("new drops:")
        #for i in range(len(drops)):
            #if not(drops[i] is None):
                #print(drops[i].name)
        
        ####WC - separate drop and steal randomization
        if monsterdrops == "n":
            drops = OGdrops
        if monsterdrops == "y":
            if random.choice([True, False]):
    ...
    #####WC - add mutate sketch/rage section
    def mutate_sketchrage(self, vanillaSketchRage=False):
        candidates = []
        def get_candidates(valid_spells, vanilla=False):
            #candidates = self.get_skillset()
            candidates = self.get_skillset(vanillaSkills=vanilla)
            #print(str(self.id) + " | " + str(candidates))
            ...
            #20% chance of adding a random spell to the candidate list
            if not vanilla:
        ...
        candidates = get_candidates(valid_spells=valid_spells, vanilla=vanillaSketchRage)
        ...
        #WC - simplify sketchces and rages
        #sketch and rage were combined for this flag because in the game there's
        #no way to actually see what these have been changed to, whereas for Control
        #you get the Control menu and can see what is available
        #update sketches. If vanilla pool only, run get_candidates, otherwise
        #use get_good_selection, which will use a candidates list that
        #already applies a 20% of having a non-vanilla spell in the list along with
        #utilizing monster AI script
        if vanillaSketchRage:
            candidates = get_candidates(valid_spells=valid_spells, vanilla=vanillaSketchRage)
            self.sketches = random.sample(candidates, min(2, len(candidates)))
        else:
            #print(candidates)
            self.sketches = get_good_selection(candidates, 2, highpower=True)
        random.shuffle(self.sketches)

        #change rages based on what the "candidates" are
        if not self.is_boss:
            if vanillaSketchRage:
                candidates = get_candidates(valid_spells=valid_spells, vanilla=vanillaSketchRage)
                self.rages = random.sample(candidates, min(4, len(candidates)))
            else:
                self.rages = get_good_selection(candidates, 2, minimum=3)
#####WC - add mutate sketch/rage section


    def mutate_control(self, vanillaControl=False):
        candidates = []
        # shuffle skills between control, sketch, rage
        #WC - switch to checking whether skills should come from vanilla skills
        def get_candidates(valid_spells, vanilla=False):
            #candidates = self.get_skillset()
            candidates = self.get_skillset(vanillaSkills=vanilla)
            #print(str(self.id) + " | " + str(candidates))
        ...
        valid_spells = [s for s in get_ranked_spells() if s.valid]
        candidates = get_candidates(valid_spells=valid_spells, vanilla=vanillaControl)

        #20% chance of adding a random spell to the candidate list
        ####WC - add in vanilla flag, move this section into get_candidates
        #for s in valid_spells:
            #print(s.name)

        #if not vanillaControl:
            #if random.randint(1, 10) >= 9:
                #sb = self.get_spell_appropriate(valid_spells)
                #candidates.add(sb.spellid)
        ...
        #EE is Battle/normal Fight, EF is special
        if 0xEE not in self.controls and 0xEF not in self.controls:
            #this guarantees that Battle/Special is one of the control options
        ...
        #WC - append to monsterblock candidates
    ...
    def mutate_easybosses(self):
        ...
        #self.mutate_stats
        #Vargas, Ultros 3, Ultros 2?
        hp4x = ["Leader"]  #same as WC
        hp3x = ["Rizopas", "Piranha", "TunnelArmr"] #same as WC
        WC_hp2x = ["Ipooh", "GhostTrain", "Dadaluma", "Ifrit", "Shiva", "Number 024",
                "Number 128", "Left Blade", "RightBlade", "Crane", "Nerapa"]
        hp2x = ["Ipooh", "Left Blade", "RightBlade"]

        speedbump = ["Leader", "Rizopas", "Piranha", "TunnelArmr", "GhostTrain", "Dadaluma",
                     "Number 024", "Number 128"]

        #5x HP
        if self.display_name in hp4x:
            self.stats['hp'] = min(self.stats['hp'] * 5/4, 0xFFFF)
            if self.display_name == "Leader":  #Leader needs more help, 10x HP
                self.stats['hp'] = min(self.stats['hp'] * 2, 0xFFFF)
        if self.display_name in hp3x:  #Rizopas, TunnelArmr
            if self.display_name != "Piranha":
                self.stats['hp'] = min(self.stats['hp'] * 5/3, 0xFFFF)
        if self.display_name in hp2x:  #Ipooh, Left Blade, RightBlade
            self.stats['hp'] = min(self.stats['hp'] * 5/2, 0xFFFF)

        #2x HP
        #Vargas, Dadaluma, Ultros 1 and 3
        if (self.display_name == "Vargas" or self.display_name == "Dadaluma" or
        self.display_name == "Number 128" or self.display_name == "GhostTrain" or
            self.id == 300 or self.id == 302):
            self.stats['hp'] = min(self.stats['hp'] * 2, 0xFFFF)
            self.stats['speed'] = 230

        if self.display_name == "Air Force":
            self.stats['hp'] = min(self.stats['hp'] * 7/4, 0xFFFF)

        if self.display_name == "Nerapa":
            self.stats['hp'] = min(self.stats['hp'] * 3/2, 0xFFFF)

        #230 speed "Leader", "Rizopas", "Piranha", "TunnelArmr", "GhostTrain", "Dadaluma", "Number 024", "Number 128"
        if self.display_name in speedbump:
            self.stats['speed'] = 230

        #Kefka @ Narshe
        if self.id == 330:
            self.stats['hp'] = min(self.stats['hp'] * 2, 0xFFFF)  #4x normal
            self.stats['speed'] = 230
            self.statuses[2] |= 0x08 #Haste
            self.statuses[2] |= 0x80 #Reflect

        #self.mutate_statuses
        if self.display_name == "Leader":
            self.statuses[2] |= 0x02 #Regen
            self.statuses[3] |= 0x02 #Runic
            #already has Auto-Safe
            
        if self.display_name == "TunnelArmr":
            self.statuses[2] |= 0x08 #Haste
            self.statuses[2] |= 0x20 #Shell
            self.statuses[2] |= 0x40 #Safe

        if self.display_name == "Rizopas":
            self.statuses[1] |= 0x04 #Image
            self.statuses[2] |= 0x20 #Shell

        if self.display_name == "Number 024":
            self.statuses[2] |= 0x20 #Shell
            self.statuses[2] |= 0x40 #Safe

        if self.display_name == "Dadaluma":
            self.statuses[2] |= 0x02 #Regen
            self.statuses[2] |= 0x20 #Shell

        #self.mutate_affinities <- look into this
        #self.mutate_special <- prob leave these alone
        #self.mutate_ai <- work on monster scripts
        #self.mutate_control
        #self.mutate_sketchrage

    #end easyboss section

    def mutate(self, Options_, change_skillset=None, safe_solo_terra=True, katn=False,monsterstats1="n", monsterstats2="n", monstermisc="n", monsterautostatus="n",
                 monsterelemental="n", monsterspecials="n", monsters_darkworld="n",
                 monsterscripts="n", monstercontrol="n", monsterdrops="n",
                 monstersteals="n", monstermorphs="n", vanillacontrol="n",
               monsterSketchRage="n", vanillaSketchRage="n"):
        ...
        if vanillacontrol == "n":
            vanillaControlFlag = False
        else:
            vanillaControlFlag = True

        #handles Sketch/Rage options
        if vanillaSketchRage == "n":
            vanillaSketchRageFlag = False
        else:
            vanillaSketchRageFlag = True


        ####WC - in order to apply BC style mutate_ai
        #AI needs to be reverted to vanilla
        if monsterscripts == "y":
            # WC - this seems to be bugged out
            # self.aiscript = self.vanilla_aiscript
            pass
        ...
        if monsters_darkworld == "y":
            darkworld = True

        #adjusts level, all stats, HP/MP/XP/GP
        self.mutate_stats(monsterstats1, monsterstats2)

        #adjusts escapability, scannable, and undead flags.
        #no way for the game to tell you undead status. escapability is somewhat handled already.
        #disabling scan might be more relevant with other flags
        if monstermisc == "y":
            self.mutate_misc()

        #adjusts default statuses and status immunities. may want to separate this out.
        #there isn't any way for the game to tell you whether an enemy is weak to/strong against status effects
        #probably want to leave this vanilla
        if monsterautostatus == "y":
            self.mutate_statuses()

        #adjusts elemental absorption, nullification, and weaknesses. Weaknesses can be scanned but defenses cannot
        #could be applied to weak enemies but pull back on harder ones. study the probabilities more.
        if monsterelemental == "y":
            self.mutate_affinities(odds=5 if madworld else 10)

        #could be interesting, study the probabilities more. this seems to affect Sketch results as well
        if monsterspecials == "y":
            self.mutate_special(darkworld, narshesafe=False, katn=False)
        
        #this affects battle script, study the probabilities more.
        #note that enemy ability scaling in WC causes the battle scripts to display
        #unexpected values in place of Specials and other "probabilistic" abilities (e.g. spells)
        if monsterscripts == "y":
            self.mutate_ai(Options_=Options_, change_skillset=True,safe_solo_terra=True,vanillaControlFlag=vanillaControlFlag)


        #############WC - BC original monster adjustment section
        if 0 == 1:
            ...
            if value > 1:
                if value == 2:
                    ...
                    self.mutate_ai(Options_=Options_, change_skillset=change_skillset,
                                   safe_solo_terra=safe_solo_terra)
        #############end of monster adjustment section

        #adjusts control, sketch, and rage options
        if monstercontrol == "y":
            self.mutate_control(vanillaControl=vanillaControlFlag)
        if monsterSketchRage == "y":
            self.mutate_sketchrage(vanillaSketchRage=vanillaSketchRageFlag)
