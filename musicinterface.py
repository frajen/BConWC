...
BC_MUSIC_FREESPACE = ["320000-3FFFFF"]

def music_init():
    johnnydmad_initialize(rng=random)
    
def randomize_music(fout, Options_, opera=None, form_music_overrides={}, chaoticmusic=False):
    events = ""
    #if Options_.is_code_active('christmas'):
        #events += "W"
    #if Options_.is_code_active('halloween'):
        #events += "H"
    #not using BC codes, comment out this line
    ...
    data = process_music(data, metadata, f_chaos=chaoticmusic, eventmodes=events, opera=opera, subpath="music", freespace=BC_MUSIC_FREESPACE, ext_rng=random)
